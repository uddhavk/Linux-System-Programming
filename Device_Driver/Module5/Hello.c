...
4. cat log.txt
    ...
    modinfo Hello.ko      --> gives all information about module

    5.sudo insmod Hello.ko MarvellousNo=51 --> user can input any value or default value will be as in program i.e 11
        sudo insmod Hello.ko MarvellousNo=51 --> to insert the user value old module should be removed using sudo rmmod filename(Hello)
